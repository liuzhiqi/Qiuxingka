...
- (void)startDownloadImage
{
   ...
   // [[SDWebImageManager sharedManager] cancelForDelegate:self];
    ...
    for (NSInteger i=0; i<self.imagesArray.count; ++i)
    {
        ...
        if ([url isNotEmpty])
        {
            [[SDWebImageManager sharedManager] downloadWithURL:[NSURL URLWithString:url] options:(0) progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
                totalCount--;
...
//            [[SxDWebImageManager sharedManager] downloadWithURL:[NSURL URLWithString:url] delegate:self];
        }
    }
}
